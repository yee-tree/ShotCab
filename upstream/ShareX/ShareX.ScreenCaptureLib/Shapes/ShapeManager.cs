    ...
    internal partial class ShapeManager : IDisposable
    {
        ...
        public ShapeType CurrentTool
        {
            ...
            set
            {
                ...
                if (Form.IsEditorMode) Form.SetDefaultCursor();
            }
        }
        ...
        public ShapeManager(RegionCaptureForm form)
        {
            ...
            {
                // Editing an existing picture begins in navigation/selection mode.
                // Remembered drawing tools must not accidentally mark the first drag.
        ...
        internal BaseShape CreateShape(ShapeType shapeType)
        {
            BaseShape shape;
            ...
            switch (shapeType)
            {
                ...
                case ShapeType.EffectSolidMask:
                    shape = new SolidMaskEffectShape();
                    break;
                case ShapeType.EffectSpotlight:
                    shape = new SpotlightEffectShape();
                    break;
        ...
        public Bitmap RenderOutputImage(Bitmap bmp, PointF offset)
        {
            ...
            if (DrawingShapes.Length > 0 || EffectShapes.Length > 0)
            {
                ...
                using (Graphics g = Graphics.FromImage(bmpOutput))
                {
                    ...
                    ShotCabEffectSnapshot = DrawingShapes.Any(shape => shape is MagnifyDrawingShape) ? new Bitmap(bmpOutput) : null;
                    try
                    {
                        foreach (BaseDrawingShape shape in DrawingShapes)
                        {
                            if (shape != null) shape.OnDraw(g);
                        }
                    }
                    finally { ShotCabEffectSnapshot?.Dispose(); ShotCabEffectSnapshot = null; }
                }
                }
                catch { bmpOutput.Dispose(); throw; }
                finally
                {
                MoveAll(offset);
