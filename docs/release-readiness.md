...
当前结论：**经筛选的源码可以公开；便携包与安装包仍需单独完成发行验收。**
...
后续安装版已列入计划：主程序与完整离线 OCR 组件应一起安装，使普通用户无需手动选择 OCR 路径。安装包制作前须先解决目前默认把历史数据写在程序目录的问题，并完成 OCR 第三方许可清单。安装目录、数据迁移、升级、卸载和验收清单见 [`installer-plan.md`](installer-plan.md)。
