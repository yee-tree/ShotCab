    ...
    internal partial class ShapeManager
    {
        ...
        internal void CreateToolbar()
        {
            ...
            if (Form.IsEditorMode)
            {
                ...
                // ShotCab has no upload workflow; retain upstream event code but no exposed button.
                tsbUploadImage.Dispose();
            ...
            foreach (ShapeType shapeType in Helpers.GetEnums<ShapeType>())
            {
                ...
                switch (shapeType)
                {
                    ...
                    case ShapeType.EffectHighlight:
                        ...
                        break;
                    case ShapeType.EffectSolidMask:
                        tsbShapeType.Text = "实心遮挡";
                        tsbShapeType.DisplayStyle = ToolStripItemDisplayStyle.Text;
                        break;
                    case ShapeType.EffectSpotlight:
                        tsbShapeType.Text = "聚光灯";
                        tsbShapeType.DisplayStyle = ToolStripItemDisplayStyle.Text;
                        break;
            ...
            Form.PrepareShotCabToolbar();
            tsMain.PerformLayout();
            menuForm.PerformLayout();

            // Place the owned palette before it becomes visible. Its old initial
            // position (200, 200) could be painted over the editor's first frame.
            UpdateMenuPosition();
            ...
            Form.Activate();
            Form.ShotCabPrimeChrome();
            if (Form.IsHandleCreated)
                Form.BeginInvoke(new Action(() => { if (!Form.IsDisposed) Form.ShotCabPrimeChrome(); }));
        ...
        {
            // Reserve the full-width palette at the window's bottom edge.
        ...
        internal void UpdateMenuPosition()
        {
            ...
            {
                rectScreen = Form.IsEditorMode ? Form.Bounds : Form.RectangleToScreen(Form.ClientRectangle);
            }
            ...
            {
                menuForm.Location = new Point(rectScreen.X, rectScreen.Bottom - menuForm.Height);
            }
            else if (tsMain.Width < rectScreen.Width)
            {
                menuForm.Location = new Point(rectScreen.X + (rectScreen.Width / 2) - (tsMain.Width / 2), rectScreen.Y + Form.ShotCabCaptionHeight);
            }
            else
            {
                menuForm.Location = new Point(rectScreen.X, rectScreen.Y + Form.ShotCabCaptionHeight);
            }
        }

        internal void UpdateMenuMaxWidth(int width)
        {
            if (Form.IsEditorMode) { ShotCabResizeToolbar(width); return; }
