...
namespace ShareX.ScreenCaptureLib
{
    internal class ScrollingCaptureManager : IDisposable
    {
        ...
        private Bitmap CombineImages(Bitmap result, Bitmap currentImage)
        {
            ...
            // A previous scroll distance is not evidence that this frame overlaps.
            // Stop and retain the accumulated result when matching is too weak.
            if (matchCount >= Math.Min(16, Math.Max(2, currentImage.Height / 10)))
            {
                ...
                if (matchHeight > 0)
                {
                    ...
                    if (status != ScrollingCaptureStatus.PartiallySuccessful)
                    {
                        status = ScrollingCaptureStatus.Successful;
                    }
