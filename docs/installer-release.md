...
状态：本机可用的安装包候选版，尚未签名或发布为 GitHub Release。生成物在 `artifacts/ShotCab-0.1.0-win-x64-setup.exe`，以构建时的 SHA-256 和 `payload-manifest.tsv` 为准。
...
尚需在干净 Windows 10/11 x64 机器上进行交互式组件页、快捷方式、缺失 .NET、正在运行时的提示、低磁盘空间、真实用户历史迁移与卸载后恢复验证。安装包当前未签名，Windows 可能显示未知发布者提示。公开二进制前须将同一提交的源码包、SHA-256 和完整第三方声明一同提供；这份本机记录不代替跨机器或法律验收。
